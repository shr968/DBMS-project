...
function authenticatePatientOrDoctor(req, res, next) {
    if (req.session.patient_id || req.session.doctor_id || req.session.hospital_id) {
        return next(); 
    }
...
app.post('/doctor-register', (req, res) => {
    ...
    db.query(findDoctorSql, [first_name, mid_name, last_name, speciality], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            db.query(doctorSql, [first_name, mid_name, last_name, speciality], (err, result) => {
                if (err) {
...
app.get('/doctor-dashboard/:doctor_id', async (req, res) => {
    ...
    try {
        ...
        res.render('doctor-dashboard', { doctor_id: doctorId, hospitals, patients });
    } catch (err) {
        console.error(err);
...
app.get('/patient-dashboard/:patient_id',authenticatePatientOrDoctor, (req, res) => {
...
app.get('/emergency/:id',authenticatePatientOrDoctor, (req, res) => {
    ...
    if (req.session.doctor_id || req.session.hospital_id || req.session.patient_id == patient_id) {
...
app.post('/add-record/:id',(req, res) => {
    ...
    db.query(sql, [patient_id, drive_link, date, hospital_name, description || null], (err, result) => {
        if (err) {
...
app.post('/treatment/add', (req, res) => {
    ...
    const sql = `INSERT INTO treatment (patient_id, doctor_id, doctor_name, hospital_id, hospital_name) 
                 VALUES (?, ?, ?, ?, ?)`;
    db.query(sql, [patient_id, doctor_id, doctor_name, hospital_id, hospital_name], (err, result) => {
        if (err) {
